...
var sale = {
    ...
    calculateInvoice: function () {
        ...
        $('input[name="total_iva"]').val(sale.details.total_iva.toFixed(0));
        $('input[name="total_dscto"]').val(sale.details.total_dscto.toFixed(0));
        $('input[name="total"]').val(sale.details.total.toFixed(0));
        $('input[name="amount"]').val(sale.details.total.toFixed(0));
...
document.addEventListener('DOMContentLoaded', function (e) {
    function validateChange() {
        ...
        if (method_payment === 'efectivo') {
            ...
            input_amount_debited.val(amount_debited.toFixed(0));
        }
        return {valid: true};
    }
...
$(function () {
    ...
    $('#tblProducts tbody')
        ...
        .on('change', 'input[name="cant"]', function () {
            ...
            $('td:eq(-4)', tblProducts.row(tr.row).node()).html('$' + sale.details.products[tr.row].subtotal.toFixed(0));
            $('td:last', tblProducts.row(tr.row).node()).html('$' + sale.details.products[tr.row].total.toFixed(0));
        })
        .on('change', 'input[name="dscto_unitary"]', function () {
            var tr = tblProducts.cell($(this).closest('td, li')).index();
            sale.details.products[tr.row].dscto = parseFloat($(this).val());
            sale.calculateInvoice();
            $('td:eq(-2)', tblProducts.row(tr.row).node()).html('$' + sale.details.products[tr.row].total_dscto.toFixed(0));
            $('td:last', tblProducts.row(tr.row).node()).html('$' + sale.details.products[tr.row].total.toFixed(0));
        })
    ...
    select_payment_method.on('change', function () {
        ...
        switch (id) {
            ...
            case "tarjeta_debito_credito":
                ...
                input_amount_debited.val(sale.details.total.toFixed(0));
                input_titular.val('');
    ...
    input_cash
        ...
        switch (paymentmethod) {
            ...
            case "efectivo":
                ...
                input_change.val(change.toFixed(0));
                fvSale.revalidateField('change');
                break;
        }
        return false;
